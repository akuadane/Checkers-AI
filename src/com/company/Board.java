...
public class Board {
    ...
    public int[][] findLegalMoves(PieceOwner owner){
        ...
        for(int r=0;r<this.board.length;r++){
            for(int c=(1-r%2);c<this.board.length;c+=2){ // Makes sure the Pieces are placed on squares where i+j is odd
                ...
                if(piece!=null && piece.owner==owner && piece.type==PieceType.PAWN)
                    this.findMoves(piece.owner,r,c);
    ...
    private void findMoves(PieceOwner owner,int r, int c){
        int[][] moveDir;

        if(owner==PieceOwner.PLAYER1)
            moveDir= new int[][]{{-1,-1},{-1,1}};
        else
            moveDir= new int[][]{{1,-1},{1,1}};
