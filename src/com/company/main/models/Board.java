...
public class Board {
    ...
    public List<Move> findLegalMoves(PieceOwner inTurnPlayer) {
        ...
        if (possibleMovements.size() == 0) possibleMovements.addAll(moveList);
    ...
    private List<Jump> findJumps(Piece piece, int r, int c) {
        ...
        for (Move mv : findMoves(piece, r, c)) {  // Use BreadthFirst search to locate all possible jumps from this point
            if (!(mv instanceof Jump)) continue;
            ...
            while (queue.size() > 0) {
                ...
                for (Move nMove : findMoves(piece, x.movement[2], x.movement[3])) {
                    if (!(nMove instanceof Jump)) continue;
    ...
    private List<Move> findMoves(Piece piece, int r, int c) {
        ...
        } else moveDir = new int[][]{{1, -1}, {1, 1}, {-1, 1}, {-1, -1}}; // allows kings to move to every direction
        ...
        for (int i = 0; i < moveDir.length; i++) {
            ...
            if (newR < 0 || newR >= this.board.length || newC < 0 || newC >= this.board.length) continue;
            ...
            else if (newPos.owner == piece.owner) continue;
    ...
    public void makeMove(Move move, PieceOwner playerInTurn) {
        ...
        if (newR == 0 && playerInTurn == PieceOwner.PLAYER1) board[newR][newC].type = PieceType.KING;

        if (newR == board.length - 1 && playerInTurn == PieceOwner.PLAYER2) board[newR][newC].type = PieceType.KING;
     ...
     */
    public void undo() {
        if (prevBoard != null) board = Arrays.copyOf(prevBoard, prevBoard.length);
    }
        ...
        for (int i = 0; i < this.board.length; i++) {
            for (int j = 0; j < this.board.length; j++) {
                ...
                if (piece != null) {
                    ...
                    if (piece.type == PieceType.KING) p = Character.toUpperCase(p);
                }
    ...
    public PieceOwner isGameOver() {
            ...
            for (int c = (1 - r % 2); c < this.board.length; c += 2) { // Makes sure the Pieces are placed on squares where i+j is odd
                ...
                if (piece != null) {
                    if (findMoves(piece, r, c).size() != 0) {
                        if (piece.owner == PieceOwner.PLAYER1) p1HasMoves = true;
        ...
        if (p1HasMoves) return PieceOwner.PLAYER1;
    ...
    public Board clone() {
        ...
        for (int i = 0; i < this.board.length; i++) {
            for (int j = (1 - i % 2); j < this.board.length; j += 2) {
                if (board[i][j] != null) tempBoard.board[i][j] = board[i][j].clone();
                else tempBoard.board[i][j] = null;
            }
        }
        return tempBoard;
    }
